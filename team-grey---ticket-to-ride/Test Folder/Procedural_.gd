...
const MIN_CITY_DIST: float = 100.0 # Increased to give cities more breathing room
...
func add_route(id1: int, id2: int):
	...
	routes.append([id1, id2, length, route_color])
...
# 3. Draw Routes (Lines/Tracks)
	for route in routes:
		...
		draw_train_route(cities[route[0]], cities[route[1]], route[2], route[3])
